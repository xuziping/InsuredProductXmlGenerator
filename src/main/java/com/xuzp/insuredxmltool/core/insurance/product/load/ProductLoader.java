...
//		String free = XmlUtil.getValue(node, "free");
	...
	private void parseData(XmlNode e, String path)
	{
		for (Iterator iter = e.getChildren().iterator(); iter.hasNext(); )
		{
			...
			if ("item".equals(n1.getName()))
			{
				...
				if (isEmpty(file) && n1.hasAttribute("value")) {
					//这个是考虑向下兼容，现在都使用file项，并要求加后缀
...
//					v.put("file", new File(path + value + ".dat"));
	...
	public static Rule ruleOf(XmlNode ruleNode)
	{
		...
		if ("customer".equalsIgnoreCase(type)) {
			...
			rule.setType(Rule.TYPE_COMBINE);
		}
	...
	private void parseInterest(XmlNode e)
	{
		...
		VariableDefine interestVars = isEmpty(script) ? new VariableDefine() : new VariableDefine(FormulaUtil.formulaOf(script));
...
//				System.out.println(value);
				...
				if (!isEmpty(param)) {
					//如果有这个值表示是一个函数，函数有入参，结果根据不同的参数会变化，所以不能缓存结果。
...
//	}
	...
	private void parsePortfolio(XmlNode e)
	{
		...
		for (Iterator iter = e.getChildren("product").iterator(); iter.hasNext(); )
		{
			...
			Insurance p = (Insurance)company.getProduct(productId);
			portfolio.addProduct(p, iv, desc);
		}
	...
	private void parseRoot(XmlNode e, String path)
	{
		for (Iterator iter = e.getChildren().iterator(); iter.hasNext(); )
		{
			...
			else if ("input".equals(n1.getName()))
			{
				parseInput(n1);
			}
